...
class SourceTimeFunction(metaclass=abc.ABCMeta):
    ...
    def convolve(self, val, t, debug=False):
        """Convolve val(t) with the source time function.

        Strategy: resample the STF to the signal's dt (dt_val), then use
        FFT convolution. The result lives on the same grid as val -- which
        is the only grid that the rest of the pipeline uses.

        Why this direction (STF → dt_val, not val → dt_stf):
        - val has nfft points; upsampling it is expensive.
        - The STF is always short; resampling it is cheap.
        - The result is consumed at dt_val (add_to_response, HDF5 output),
          so computing it at a finer grid provides no benefit.
        - For all physical STFs in this pipeline the STF duration >> dt_val,
          so the resampled STF retains all relevant shape information.

        Compared to the original implementation:
        - One interpolation instead of two (no round-trip).
        - fftconvolve O(n log n) instead of convolve O(n²).
        - No intermediate upsampled arrays of size nfft * ratio.
        """
        if len(self.data) == 1:
        ...
        # Resample the STF to the signal's dt.
        # self.t may have a finer resolution (e.g. Brune generates self.t
        # at self._dt/10 for internal waveform accuracy). We bring it to
        # dt_val here -- one interpolation, no round-trip.
        t_stf_r = np.arange(self.t[0], self.t[-1] + dt, dt)
        stf_r   = interp1d(self.t, self.data,
                            bounds_error=False, fill_value=0.0)(t_stf_r)

        # FFT convolution: O(n log n). Result directly on the val grid.
