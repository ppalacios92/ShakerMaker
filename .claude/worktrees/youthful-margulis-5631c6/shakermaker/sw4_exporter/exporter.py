...
class SW4Exporter:
    ...
    def write(self):
        ...
        # ShakerMaker model stations (optional)
        ...
        # Topography surface stations (always written when topo is present)
        if topo_points_local is not None:
            ...
            # Stations between topography surface and z=0 plane (optional)
            if self.config.write_topography_z0_stations:
                receiver_lines += topography_z0_receiver_lines(
                    topo_points_local,
                    h=self.config.h,
                    start_index=len(receiver_lines) + 1,
                    prefix=self.config.station_prefix,
        ...
        # Regular grid receivers in a sub-domain (optional)
        if self.config.domain_sw4 and self.config.domain_sw4_size is not None:
            receiver_lines += domain_receiver_lines(
                self.config.h,
